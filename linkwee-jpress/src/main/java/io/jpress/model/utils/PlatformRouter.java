...
public class PlatformRouter extends RouterConverter {
	...
	public static String getRouter(Platform platform) {

		String url = getRouterWithoutSuffix(platform);
	...
	public static String getRouter(Platform platform, int pageNumber) {
		String url = getRouterWithoutSuffix(platform);
	...
	private static String getRouterWithoutSuffix(Platform platform) {
		String settingType = getRouterType();
		// 模型SLUG
		if (TYPE_STATIC_MODULE_SLUG.equals(settingType)) {
			return SLASH + "platform" + SLASH + platform.getSlug();
		}
		// 模型ID
		else if (TYPE_STATIC_MODULE_ID.equals(settingType)) {
			return SLASH + "platform" + SLASH + platform.getId();
		}
		// 日期SLUG
		else if (TYPE_STATIC_DATE_SLUG.equals(settingType)) {
			SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
			return SLASH + sdf.format(platform.getCreated()) + SLASH + platform.getSlug();
		}
		// 日期ID
		else if (TYPE_STATIC_DATE_ID.equals(settingType)) {
			SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
			return SLASH + sdf.format(platform.getCreated()) + SLASH + platform.getId();
		}
		// 前缀SLUG
		else if (TYPE_STATIC_PREFIX_SLUG.equals(settingType)) {
			String prefix = getRouterPrefix();
			return SLASH + prefix + SLASH + platform.getSlug();
		}
		// 前缀ID
		else if (TYPE_STATIC_PREFIX_ID.equals(settingType)) {
			String prefix = getRouterPrefix();
			return SLASH + prefix + SLASH + platform.getId();
		}
		// 动态ID
		else if (TYPE_DYNAMIC_ID.equals(settingType)) {
			String prefix = getRouterPrefix();
			return SLASH + prefix + "?id=" + platform.getId();
		} 
		// 动态SLUG
		else if (TYPE_DYNAMIC_SLUG.equals(settingType)) {
			String prefix = getRouterPrefix();
			return SLASH + prefix + "?slug=" + platform.getSlug();
		}
		else {
			return Consts.ROUTER_CONTENT + "?id=" + platform.getId();
		}
	}
	...
	public static String getRouterPrefix() {
		...
		if (StringUtils.isBlank(prefix))
			prefix = Consts.ROUTER_PLATFORM.substring(1);
		return prefix;
	}
	...
	public static String getContentRouterPreffix(TplModule module) {
		...
		if (Consts.MODULE_PLATFORM.equals(module.getName())) {
			return SLASH;
		}
