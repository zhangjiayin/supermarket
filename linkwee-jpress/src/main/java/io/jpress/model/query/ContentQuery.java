...
public class ContentQuery extends JBaseQuery {
	...
	public Page<Content> paginateInNormalByPid(int page, int pagesize, String module, String platformId,
			String orderBy) {
		...
		if(StringUtils.isNotBlank(platformId)){
			sql.append(" and c.platform_id = ").append(platformId);
		}
		sql.append(" order by c.created desc");
		...
		return DAO.getFromListCache(buildKey(currentContent.getModule(), "next:" + currentContent.getId()),
				new IDataLoader() {
					...
					public Object load() {
						...
						if(currentContent.getPlatformId()!=null && currentContent.getPlatformId().intValue()>0){
							sqlBuilder.append(" AND c.platform_id = ");
							sqlBuilder.append(currentContent.getPlatformId());
						}
	...
	public List<Content> queryContentByPlatformId(String platformId) {
		StringBuffer sql = new StringBuffer("select c.title,c.id,c.platform_id  from tjp_content c where c.status = 'normal' and c.platform_id =");
		sql.append(platformId);
		sql.append(" order by c.created desc limit 6");
		return DAO.find(sql.toString());
	}
	
	public List<Content> queryHotContent(int rows) {
		StringBuffer sql = new StringBuffer("select c.*  from tjp_content c where c.status = 'normal'");
		sql.append(" order by c.view_count desc");
