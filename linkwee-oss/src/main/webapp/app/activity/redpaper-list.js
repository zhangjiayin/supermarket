...
$(document).ready(function() {
	...
	$("#sendRedpaperModal").on("hidden.bs.modal", function() {
		var fileLcs = document.getElementById('fileLcs');  
		fileLcs.outerHTML=fileLcs.outerHTML;  //清空上传文件路径地址
		var fileInvestor = document.getElementById('fileInvestor');  
		fileInvestor.outerHTML=fileInvestor.outerHTML;  //清空上传文件路径地址
    ...
    editor = new $.fn.dataTable.Editor( {
        ...
        fields: [
            ...
            },
            {
                label: "红包描述:",
                name: "remark",
                fieldInfo: "描述必须以英文逗号分隔,逗号之前为期限描述,逗号之后为使用金额描述.如:'180天期以上,10000元以上'"
            },
            {
                label: "红包金额:",
                name: "money"
     ...
     table = $('#dtable').DataTable( {
        ...
        "columns": [
            ...
            { "data": "id" ,"render": function ( data, type, row ) {
            	...
            	var a = '<a href="#" class="btn btn-default btn-sm J_sendWriteListUsers" data-param='+JSON.stringify(param)+'>发送白名单用户</a>  '; //
	            var b = '<a href="#" class="btn btn-default btn-sm ui-redirect J_redpaperEdit" data-title="红包编辑" data-url="rest/redpaper/getRedpacketEditPage?activityId='+row.activityId+'&redpacketTypeId='+row.redPaperType+'">编辑</a>  ';
	            //var c = '<a href="rest/redpaper/queryRedpaperInfo" onclick="details(\''+ row.id + '\')">详情及统计</a>   ';
	            var c = '<a href="#" class="btn btn-default btn-sm ui-redirect J_redpaperDetail" data-title="红包详情及统计" data-url="rest/redpaper/queryRedpaperInfo?activityId='+row.activityId+'&redPacketTypeId='+row.redPaperType+'">详情及统计</a> ';
	            var d = '<a href="#" id='+row.redPaperType+' class="btn btn-danger btn-sm J_sendUsers" data-param='+JSON.stringify(param)+' >正式发送</a> ';
                return a+b+c+d;
             }}
        ],
    ...
    //给白名单发红包
     ...
     $("body").on("click","a.J_sendUsers",function(event){
    	 ...
    	 $("#uploadLcsForm").hide(); //隐藏file上传标签
    	 $("#uploadInvestorForm").hide(); //隐藏file上传标签
    ...
    //单选框点击事件  匹配第四个单选框
    $("input:radio[name='lcsType']:eq(3)").click(function(){
    	$("#options").hide(); //隐藏理财师级别复选框
    	$("#uploadLcsForm").hide(); //隐藏file上传标签
    	$("#uploadInvestorForm").show(); //显示上传栏
    	$("input[type=checkbox]").attr("checked",false); //理财师级别复选框不选中
   ...
   //正式发送红包
 	$("#sendRedpaper").click(function() {
 		...
 		var rdoVal = rdoCheck.val(); //all  lcsByLevel importlcs importInvestor
 		//拼接请求参数
 		var sendData = window['sendRedpaperData'];
 		sendData = jQuery.parseJSON(sendData);
 		var sendNums = $("input[type=text][name=sendNums]").val();
 		if(sendNums>=1 && sendNums<=5){
 		   //alert("是数字");
 		   sendData.sendNums = sendNums; //发送红包数
 		}else{
 		   layer.msg('发放数量请输入[1-5]数字',{time: 1000,icon: 0});
 		   return;
 		}
 		if(rdoVal == "importlcs" ){ //导入理财师信息发放红包
 			if($('#fileLcs').val()==""){
 				layer.msg("上传文件不能为空！",{time: 2000,icon: 0});
 				return;
 			} 
 			
 			var file = $('#fileLcs').val(); //获取文件名
 	 		var ext = file.substring(file.lastIndexOf(".")+1).toLowerCase(); //获取文件后缀名
 	 		if(ext != "xls"){
 	 			layer.msg("不是Excel文件！",{time: 2000,icon: 0});
 				return;
 	 		}
 			
 			var formData = new FormData();
 	 		formData.append('file', $('#fileLcs')[0].files[0]);
 	 		formData.append('activityId',sendData.activityId);
 	 		formData.append('redPacketTypeId',sendData.redPacketTypeId);
 	 		formData.append('sendNums',sendData.sendNums);
 	 		
 	 		$.ajax({
 	 		    url: './rest/redpaper/importLcs',
 	 		    type: 'POST',
 	 		    cache: false,
 	 		    data: formData,
 	 		    processData: false,
 		...
 		if(rdoVal == "importInvestor" ){ //导入金服用户信息发放红包
 			if($('#fileInvestor').val()==""){
 				layer.msg("上传文件不能为空！",{time: 2000,icon: 0});
 				return;
 			} 
 			
 			var file = $('#fileInvestor').val(); //获取文件名
 	 		var ext = file.substring(file.lastIndexOf(".")+1).toLowerCase(); //获取文件后缀名
 	 		if(ext != "xls"){
 	 			layer.msg("不是Excel文件！",{time: 2000,icon: 0});
 				return;
 			...
 			var formData = new FormData();
 	 		formData.append('file', $('#fileInvestor')[0].files[0]);
 	 		formData.append('activityId',sendData.activityId);
 	 		formData.append('redPacketTypeId',sendData.redPacketTypeId);
 	 		formData.append('sendNums',sendData.sendNums);
 	 		
 	 		$.ajax({
 	 		    url: './rest/redpaper/importInvestor',
 	 		    type: 'POST',
 	 		    cache: false,
 	 		    data: formData,
 	 		    processData: false,
