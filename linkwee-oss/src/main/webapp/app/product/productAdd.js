using(["messager","form"],function(){
	 ...
	 function checkForm(){
		 ...
		 if($("#ransferProtocalUrl").val().length> 0){//收益转让协议名称不为空  链接地址必填
		   ...
		   /*if($("#invDtlPageDes").val().length>15){
			   $.messager.alert('提示','金服端 详情推荐语最多15个字符','info');
			   $("#invDtlPageDes").focus();
			   return false;
		   }*/
		   //理财师端 详情推荐语 隐藏
		 ...
		 if($("input[name='isCollect']:checked").val() =='2'){//进行募集
			 if($("#collectEndTime").length> 0){//募集有效期不能为空 
				 if($("#collectEndTime").val().length<1){
					 $.messager.alert('提示','募集有效期不能为空','info');
					   $("#collectEndTime").focus();
					   return false;
				 }
			 }
		 }
		 ...
		 if($("#productTypeId").length> 0){//
			 if($("#collectLineMinValue").val() != $("#f_line_min_value").val() || $("#collectLineMaxValue").val() != $("#f_line_max_value").val()){
				 $.messager.alert('提示','收益模板天数:'+$("#f_line_min_value").val()+'~'+$("#f_line_max_value").val()+'与投资期限'+$("#collectLineMinValue").val()+'~'+$("#collectLineMaxValue").val()+'不符','info');
				 return false;
			 }
		 }
	...
	$("#productProtocalAddBtn").click(function(){
		if($("#productProtocalId").val().length>0){
			$("#productProtocalDiv").html($("<a>").text("收益转让协议").attr({target:"_blank",href:$("#productProtocalId").find("option:selected").text() })[0].outerHTML);
		}else{
	...
	$("#ransferProtocalUrlAddBtn").click(function(){
		if($("#ransferProtocalUrl").val().length>0){
			$("#ransferProtocalUrlDiv").html($("<a>").text($("#ransferProtocalName").val()).attr({target:"_blank",href:$("#ransferProtocalUrl").val()})[0].outerHTML);
		}else{
