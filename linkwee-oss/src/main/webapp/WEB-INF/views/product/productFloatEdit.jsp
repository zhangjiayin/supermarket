...
<div >
    ...
    <legend>基本信息</legend>
		...
		<div class="prdAdd" style="margin-top: 40px">
			...
			<span class="rightsp"><input  class="easyui-validatebox" type="text" disabled="disabled" id="productName" name="productName" data-options="required:true" id="productName" value="${dtl.productName }"><div id="proNameValidDiv" style="display: inline;"></div></span>
		</div>
...
<!-- 		<div class="prdAdd">
		...
		<div class="prdAdd">
			...
			<span class="rightsp"><input class="easyui-numberbox" type="text" name="buyTotalMoney" disabled="disabled" data-options="required:true,precision:2" value="<fmt:formatNumber value="${dtl.buyTotalMoney }" pattern="0.00"/>">
			<form:errors path="buyTotalMoney" cssClass="error" /> 
			</span>
		</div>
		<div class="prdAdd">
			<span class="leftsp">起投金额：</span>
			<span class="rightsp"><input class="easyui-numberbox" disabled="disabled" type="text" name="buyMinMoney"  missingMessage="" data-options="required:true,precision:2" value="<fmt:formatNumber value="${dtl.buyMinMoney }" pattern="0.00"/>">
			<form:errors path="buyMinMoney" cssClass="error" /> 
			</span>
		</div>
		<div class="prdAdd">
			<span class="leftsp">递增金额：</span>
			<span class="rightsp"><input class="easyui-numberbox" disabled="disabled" type="text" name="buyIncreaseMoney"  data-options="required:true,precision:2" value="<fmt:formatNumber value="${dtl.buyIncreaseMoney }" pattern="0.00"/>">
			<form:errors path="buyIncreaseMoney" cssClass="error" /> 
			</span>
		</div>
		<div class="prdAdd">
			<span class="leftsp">单用户购买上限：</span>
			<span class="rightsp"><input class="easyui-numberbox" type="text" disabled="disabled" name="custBuyMaxMoney" data-options="precision:2" value="<fmt:formatNumber value="${dtl.custBuyMaxMoney }" pattern="0.00"/>">
		...
		<div class="prdAdd">
			...
			<span class="rightsp">
			<input class="easyui-numberbox" type="text" name="collectLineMinValue" data-options="required:true" value="${dtl.collectLineMinValue }" disabled="disabled"> ~ <input class="easyui-numberbox" type="text" name="collectLineMaxValue" data-options="required:true" disabled="disabled" value="${dtl.collectLineMaxValue }">天
			<form:errors path="custBuyMaxMoney" cssClass="error" /> 
			</span>
		</div>
		...
		<div class="prdAdd">
			...
			<span class="rightsp">
			<!-- <input name="interestWayType" type="radio"  value="1" disabled="disabled"> -->募集完成后
			<select name="interestWay" disabled="disabled">
			<option value='2' <c:if test="${dtl.interestWay eq 2}">selected="selected"</c:if> >T+1</option>
			</select>起息&nbsp;&nbsp;
			 <%-- <input name="interestWayType" type="radio"  value="2" id="indexRecoment" disabled="disabled"> 固定日期起息
			 <input class="easyui-validatebox"  name="validBeginDate" disabled="disabled" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})"  style="width:150px" value="<fmt:formatDate value="${dtl.validBeginDate }" pattern="yyyy-MM-dd" />" > --%>
			</span>
		</div>
		...
		<div class="prdAdd">
			...
			<span class="rightsp"><input class="easyui-validatebox"  name="collectEndTime" id="collectEndTime" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" disabled="disabled" style="width:150px" data-options="required:true" value="<fmt:formatDate value="${dtl.collectEndTime }" pattern="yyyy-MM-dd" />">
			</span>
		</div>
		<div class="prdAdd">
			<span class="leftsp">投资客户募集期间收益：</span>
			<span class="rightsp"><input name="hasCollectRate" type="radio"  value="1" disabled="disabled" <c:if test="${! empty dtl.collectRate && dtl.collectRate >0}">checked="checked"</c:if>>募集期间有收益<input type="text"  disabled="disabled" style="width:80px; " name="collectRate" id="collectRate" value="<c:if test="${! empty dtl.collectRate && dtl.collectRate >0}"><fmt:formatNumber value="${dtl.collectRate }" pattern="0.00"/></c:if>"/>%
			&nbsp;&nbsp;<input name="hasCollectRate" type="radio"  value="0" disabled="disabled" <c:if test="${ empty dtl.collectRate || dtl.collectRate ==0  }">checked="checked"</c:if>>募集期间无收益
			</span>
		</div>
		<div class="prdAdd">
			<span class="leftsp">理财师募集期间佣金：</span>
			<span class="rightsp"><input name="hasCollectRatio" disabled="disabled" type="radio"  value="1" <c:if test="${! empty dtl.collectRatio  && dtl.collectRatio > 0}">checked="checked"</c:if>>募集期间有佣金<input type="text" style="width:80px; " disabled="disabled" name="collectRatio" id="collectRatio" value="<c:if test="${! empty dtl.collectRatio  && dtl.collectRatio > 0}"><fmt:formatNumber value="${dtl.collectRatio }" pattern="0.00"/></c:if>"/>%
			&nbsp;&nbsp;<input name="hasCollectRatio" type="radio" disabled="disabled"  value="0" <c:if test="${empty dtl.collectRatio || dtl.collectRatio ==0 }">checked="checked"</c:if> >募集期间无佣金
			</span>
		</div>
    ...
    <legend>金服端信息</legend>
    	<div class="prdAdd">
			...
			<select name="productTypeId" id="productTypeId" style="width:120px; " disabled="disabled">
    ...
    <legend>理财师端信息</legend>
    	<div class="prdAdd">
			...
			<span class="rightsp"><input  class="easyui-numberbox"  disabled="disabled" type="text" name="feeRatio" data-options="min:0,precision:2,required:true,validType:'number'" value="<fmt:formatNumber value="${dtl.feeRatio }" pattern="0.00"/>">%*支持两位小数</span>
		</div>
		<div class="prdAdd">
			<span class="leftsp">销售奖励：</span>
			<span class="rightsp"><input  class="easyui-numberbox" disabled="disabled" type="text" name="saleReward" data-options="min:0,precision:2,validType:'number'" value="<fmt:formatNumber value="${dtl.saleReward }" pattern="0.00"/>">% *支持两位小数</span>
		</div>
		<div class="prdAdd">
			<span class="leftsp">显示标签：</span>
