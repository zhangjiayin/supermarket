		...
		<div class="col-sm-10">
			...
			<form id="xnewsForm" action="rest/news/save" method="post">
				...
				<div class="form-group">
					<label>应用端口</label>
					<select id="appType" class="form-control" name="appType" disabled>
				...
				<div class="form-group">
					...
					<select  name="typeCode" class="form-control">
						<c:forEach var="item" items="${newsTypeList }"  varStatus="dn">
							<option <c:if test="${(not empty requestScope.news and requestScope.news.typeCode eq item.key)  }"> selected="selected"</c:if> value="${item.key }">${item.value }</option>
						</c:forEach>
					</select>
				</div>
				<div class="form-group">
