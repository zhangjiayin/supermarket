...
<c:if test="${!empty dtl}">
	...
	<div class="row">
		<div  class="col-lg-4">理财师：<span style="color: #0b97c4;">${dtl.name}&nbsp;&nbsp;${dtl.mobile}</span></div>
	</div>
	<div class="uk-blank"></div>

	<c:if test="${!empty dtl.image}">
		<div class="row" id="wrapper_head_image">
			<div class="page-header">
				<h4><small>头像</small></h4>
			</div>
			<div class="row">
				<div class="col-sm-8"><img src="${dtl.image}" class="img-thumbnail" width="200" /></div>
	...
	<div class="row">
	    ...
	    <div class="row">
			<div class="col-sm-8">
				...
				<c:if test="${empty dtl.idcard}" >
					<c:if test="${empty dtl.name}" ><a  style="color: red;">暂未绑定</a></c:if>
					<c:if test="${!empty dtl.name}" >${dtl.name}<a  style="color: red;">[已绑定]</a></c:if>
				</c:if>
				<c:if test="${!empty dtl.idcard}">
					${dtl.name}
				</c:if>
		...
		<div class="row" >
			<div class="col-sm-6"><span>身份证：</span>
				<span id="user_certno"><label style="color: red;">未获取到...</label></span>
			</div>
			<div class="col-sm-6"></div>
		</div>
		...
		<div class="row" >
			<div class="col-sm-6"><span>银行卡：</span>
				<span id="user_bankno"><label style="color: red;">未获取到...</label></span>
			</div>
			<div class="col-sm-6"></div>
		</div>
	...
	<div class="row">
		...
		<div class="row">
			...
			<div class="col-sm-4"><a class="btn btn-primary J_history" href="rest/lcsList/querycfphistory/${dtl.mobile}">职级变更记录</a></div>
		</div>
		<div class="uk-blank"></div>
		<div class="row">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="J_formchange" method="post" action="rest/lcsList/resetpwd">
						...
						<div class="form-group">
							<label for="newpwd">${dtl.name}</label>
						</div>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="J_formChangeParent" method="post" action="rest/lcsList/changeParent">
						<div class="form-group">
							<label for="newpwd">该操作将使理财师 ${dtl.name}${dtl.mobile} 与上级 ${dtl.parentName} ${dtl.parentMobile} 解除关系，请选择解绑后理财师归属</label>
						</div>
...
<script type="text/javascript">
	...
	var $cfp_number = "${dtl.number}";
	var $customerId = "${dtl.customerId}";
//	$customerId = "eb2c38d5-fe7c-4e61-af1f-0f8869661005";
	...
	var loadUserTcInfo = function () {
		$.get("rest/lcsList/findtcuserinfo",{fid:$customerId},function (result) {
			...
			if(result.bankCode!="NOTFOUND"){

				if(result.bankName =="NOTFOUND"){
					$("#user_bankno").html(result.bankCode);
				}
				else{
...
<script type="text/javascript">
	...
	$(".J_search_cfp").click(function () {
		...
		if(!/^(13[0-9]|15[0|3|6|7|8|9]|18[8|9]|17[0|1])\d{8}$/.test(mobile)){
