...
public class TCFeeServiceImpl extends GenericServiceImpl<TCFee, Long> implements TCFeeService{
	...
	public void saveFees(FeedetailWrapper... feedetailWrappers){
		...
		for (FeedetailWrapper feedetailWrapper : feedetailWrappers) {
			...
			fee.setProfitCfplannerId(feedetailWrapper.getProfitCfplannerId());
			fee.setYearpurAmount(feedetailWrapper.getYearPurAmount());
			fee.setFeeAmount(feedetailWrapper.getFeeamount());
			fee.setFeeRate(new BigDecimal(feedetailWrapper.getRatio()));
			fee.setFeeType(feedetailWrapper.getFeetype());
			fee.setRemark(feedetailWrapper.getRemark());
			fee.setUpdateTime(date);
			fee.setLowUserId(feedetailWrapper.getLowUserId());
			fee.setLowType(feedetailWrapper.getLowType());
			if(feeMapper.isExitFee(billId,fee.getProfitCfplannerId(),feedetailWrapper.getFeetype())){
				...
				fee.setOriginCfplannerId(feedetailWrapper.getOriginCfplannerId());
	...
	@Override
	public Double queryOfflineFeeByUserId(String userId, String month) {
		return feeMapper.queryOfflineFeeByUserId(userId, month);
	}
	...
	@Override
	public Double queryLeaderRewardByUserIdNew(String userId, String month) {
		return feeMapper.queryLeaderRewardByUserIdNew(userId, month);
	}
	...
	public String productProfitCalculate(ProductDetailResponse productDetail) {
		...
		return String.format("现在投资%s,%s天后收益%s元",amtStr,productDetail.getDeadLineMinValue(),String.valueOf(BigDecimalUtil.scale(productProfitCalculate.setScale(2,BigDecimal.ROUND_DOWN).doubleValue())));
	}


	@Override
	public Double queryDirectAllowanceByUserId(String userId, String month) {
		return feeMapper.queryDirectAllowanceByUserId(userId, month);
	}


	@Override
	public Double queryTeamAllowanceByUserId(String userId, String month) {
		return feeMapper.queryTeamAllowanceByUserId(userId, month);
	}
