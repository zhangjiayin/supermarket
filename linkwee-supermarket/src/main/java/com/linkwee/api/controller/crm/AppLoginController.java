	...
	public BaseResponse gesturePwdLogin(AppRequestHead requestHead){
		...
		if(StringUtils.isBlank(requestHead.getToken())){
			...
			if(jsonWebTokenHepler.resetTokenValidDate(requestHead.getAppKind(),requestHead.getToken())){
				//更新最新一次登录设备信息
				String orgNumber = requestHead.getOrgNumber();
				customerDeviceService.recordCustomerDeviceInfo(JsonWebTokenHepler.getUserIdByToken(requestHead.getToken()),AppUtils.getPlatform(orgNumber).getValue(),AppUtils.getAppType(orgNumber).getKey(), null);
				return	AppResponseUtil.getSuccessResponse();
			} else {
				return	AppResponseUtil.getErrorToken(new BaseResponse("token_isNotExist","token不存在"));
			}
		}
	}
