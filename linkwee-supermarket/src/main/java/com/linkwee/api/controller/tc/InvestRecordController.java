...
public class InvestRecordController {
	...
	public Object queryCustomerInvestRecord(@Valid CustomerInvestRecordRequest req,BindingResult validResult,AppRequestHead head){
		...
		if(!AppUtils.isInvestorApp(head.getOrgNumber()))return AppResponseUtil.getErrorBusi("100002", "请使用正确的app类型");
		req.setPageSize(30);//页面默认显示30条数据
		try{
			Page<InvestRecordResponse> page  = new Page<InvestRecordResponse>(req.getPageIndex(),req.getPageSize()>10?30:req.getPageSize()); //默认每页30条
	...
	@RequestMapping("customer/investProfit")
	@ResponseBody
	@RequestLogging("客户收益")
	public Object getInvestProfit(AppRequestHead head){
		if(!AppUtils.isInvestorApp(head.getOrgNumber()))return AppResponseUtil.getErrorBusi("100002", "请使用正确的app类型");
		try{
			return AppResponseUtil.getSuccessResponse(productInvestRecordService.getInvestProfit(JsonWebTokenHepler.getUserIdByToken(head.getToken())));
			
			
		}catch(Exception e){
			LOGGER.error("customer getInvestProfit Exception" , e);
		}
		return new BaseResponse(-1,"查询失败，请联系客服");
	}
