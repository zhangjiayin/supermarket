...
import com.linkwee.api.request.mc.CustomerSignPushRequest;
...
public class MsgController  {
	...
	public BaseResponse bulletinPageList(PaginatorRequest req,AppRequestHead head) throws Exception {
        ...
        if(StringUtils.isNotBlank(userId) && !userId.equals("undefined")){
        	...
        	}else if(appType != null && appType.intValue() == AppTypeEnum.CHANNEL.getKey()){
        		CrmCfplanner cfp = crmCfplannerService.queryCfplannerByUserId(userId);
	        	if(cfp!= null && cfp.getCreateTime() != null ){
	        		conditions.put("regTime", DateUtils.format(cfp.getCreateTime(), DateUtils.FORMAT_LONG));
	        	}
        	}
     ...
     * @throws Exception
     */
	@RequestLogging("设置签到推送状态")
	@RequestMapping("/setSignPushStatus/4.6.6")
	@ResponseBody
	public BaseResponse setSignPushStatus(@Valid CustomerSignPushRequest req, BindingResult result, AppRequestHead head) throws Exception {
		if (AppResponseUtil.existsParamsError(result)) {
			return AppResponseUtil.getErrorParams(result);
		}
		String userId = JsonWebTokenHepler.getUserIdByToken(head.getToken());
		...
		SmCustomerDevice customerDevice = customerDeviceService.queryCustomerDevice(appType, userId);
		if(null != customerDevice){
            customerDevice.setSignPushStatus(req.getSignPushStatus());
			customerDeviceService.doDeviceInfo(customerDevice);
		}else{
			AppResponseUtil.getErrorBusi("device_not_found", "未找到该设备信息，设置失败");
		}
     ...
     * @throws Exception
     */
    @RequestLogging("查询签到推送状态")
    @RequestMapping("/querySignPushStatus/4.6.6")
    @ResponseBody
    public BaseResponse querySignPushStatus(AppRequestHead head) throws Exception {
        String userId = JsonWebTokenHepler.getUserIdByToken(head.getToken());
        Integer appType = AppUtils.getAppType(head.getOrgNumber()).getKey();
        SmCustomerDevice customerDevice = customerDeviceService.queryCustomerDevice(appType, userId);
        Map<String,Object> resultMap = new HashMap<String, Object>();
        if(customerDevice != null){
			resultMap.put("signPushStatus",customerDevice.getSignPushStatus());
		}else {
			resultMap.put("signPushStatus",2);
		}
        return AppResponseUtil.getSuccessResponse(resultMap);
    }
}
