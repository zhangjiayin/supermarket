...
import com.linkwee.api.response.crm.CrmCfpNewcomerTaskNewResponse;
...
public class CrmCfpNewcomerTaskController extends BaseController {
	 ...
	 * @return
	 */
	@RequestMapping("homepage/2.2")
	@ResponseBody
	public BaseResponse homePageNew(AppRequestHead head) {
		String userId = JsonWebTokenHepler.getUserIdByToken(head.getToken());
		CrmCfpNewcomerTask req = new CrmCfpNewcomerTask();
		req.setUserId(userId);
		CrmCfpNewcomerTask rlt = crmCfpNewcomerTaskService.selectOne(req);
		...
		return AppResponseUtil.getSuccessResponse(rlt, CrmCfpNewcomerTaskNewResponse.class);
	}
