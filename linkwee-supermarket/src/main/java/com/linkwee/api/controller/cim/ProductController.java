...
public class ProductController {
	...
	public BaseResponse productDetail(AppRequestHead head,String productId){
		...
		productDetailRequest.setUserId(userId);
		...
		if(productDetailResponse != null && StringUtils.isNotBlank(userId) && !"undefined".equals(userId)){
			...
			if(head.getAppVersion().compareTo("4.5.6") > 0){//4.5版本以下的不显示优惠券		
				//可用优惠券数量
				ProductAddfeeTicketRequest productAddfeeTicketRequest = new ProductAddfeeTicketRequest();
				productAddfeeTicketRequest.setProductId(productId);
				productAddfeeTicketRequest.setUserId(userId);
				productDetailResponse.setCouldUsePersonAddfeeTicketCounts(actPersonAddfeeTicketService.productPersonAddfeeTicketCount(productAddfeeTicketRequest));
			}
		}
	 ...
	 */
	@ResponseBody
	@RequestLogging("产品列表4.0")
	@RequestMapping("/productPageList/4.0")
	public BaseResponse productPageList4(AppRequestHead appRequestHead,ProductPageList4Request productPageList4Request){
		LOGGER.info("查询理财产品列表4.0, productPageList4Request={}", JSONObject.toJSONString(productPageList4Request));
		PaginatorResponse<ProductPageList4Response> rlt = cimProductService.queryProductPageList4(appRequestHead,productPageList4Request);
		return AppResponseUtil.getSuccessResponse(rlt);
	}
	 ...
	 */
	@ResponseBody
	@RequestLogging("产品列表统计4.0")
	@RequestMapping("/productPageListStatistics/4.0")
	public BaseResponse productPageListStatistics4(AppRequestHead appRequestHead,ProductPageList4Request productPageList4Request){
		LOGGER.info("查询理财产品列表统计4.0, productPageList4Request={}", JSONObject.toJSONString(productPageList4Request));
		Map<String, Integer> returnMap = new HashMap<String, Integer>();
		returnMap.put("count", cimProductService.productPageListStatistics4(appRequestHead,productPageList4Request));
	 ...
	 */
	@ResponseBody
    @RequestMapping("/selectedProducts/4.0.0")
    @RequestLogging("精选产品")
	public BaseResponse querySelectedProducts(String limitNumber,AppRequestHead head){
		ProductPageListRequest productPageListRequest =  new ProductPageListRequest();
    	String userId = JsonWebTokenHepler.getUserIdByToken(head.getToken());//查询userId
    	productPageListRequest.setAppKind(AppUtils.getAppType(head.getOrgNumber()).getValue());
    	productPageListRequest.setIfHaveGray(sysGrayReleaseService.ifHaveGrayPermission(userId, "0,1"));
    	if(limitNumber != null){   		
    		try {
    			productPageListRequest.setLimitNumber(Integer.parseInt(limitNumber));
			} catch (NumberFormatException e) {
				LOGGER.info("精选产品类型转为int出错，limitNumber={}",limitNumber);
				return AppResponseUtil.getErrorBusi("100002", "参数错误");
			}   		
    	}
		List<ProductPageList4Response> rlt = cimProductService.querySelectedProducts(head,productPageListRequest);
		return AppResponseUtil.getSuccessResponse(rlt);
	}
	 ...
	 */
	@ResponseBody
    @RequestMapping("/selectedProductsList/4.1.1")
    @RequestLogging("精选产品列表")
	public BaseResponse querySelectedProductsList(AppRequestHead head,SelectedProductsListRequest selectedProductsListRequest){
		LOGGER.info("查询精选产品列表, selectedProductsListRequest={}",JSONObject.toJSONString(selectedProductsListRequest));
		selectedProductsListRequest.setAppKind(AppUtils.getAppType(head.getOrgNumber()).getValue());
    	selectedProductsListRequest.setIfHaveGray(sysGrayReleaseService.ifHaveGrayPermission(JsonWebTokenHepler.getUserIdByToken(head.getToken()), "0,1"));//灰度权限管控
    	PaginatorResponse<ProductPageList4Response> rlt = cimProductService.querySelectedProductsList(head,selectedProductsListRequest);
		return AppResponseUtil.getSuccessResponse(rlt);
	}
	...
	@ResponseBody
	@RequestLogging("产品红包")
	@RequestMapping("/productRedPacket")
	public BaseResponse productRedPacket(AppRequestHead head,ProductRedPacketRequest productRedPacketRequest){
		LOGGER.info("查询产品可使用的红包, productRedPacketRequest={}", productRedPacketRequest);
		//可使用的红包数量
		String userId = JsonWebTokenHepler.getUserIdByToken(head.getToken());//查询userId
		if(StringUtils.isNotBlank(userId) && !"undefined".equals(userId)){
			productRedPacketRequest.setUserId(userId);
			return AppResponseUtil.getSuccessResponse(redPacketService.productRedPacket(productRedPacketRequest));
		} else {
   ...
   /**
	 ...
	 */
	@ResponseBody
	@RequestLogging("产品分类统计4.6.0")
	@RequestMapping("/productClassifyStatistics/4.6.0")
	public BaseResponse productClassifyStatistics460(AppRequestHead appRequestHead,ProductStatisticsRequest productStatisticsRequest){
		List<ProductStatistics460Response> productStatisticsResponses= cimProductService.productClassifyStatistics460(appRequestHead,productStatisticsRequest);
		return AppResponseUtil.getSuccessResponse(productStatisticsResponses);
	}
	...
	@ResponseBody
	@RequestLogging("产品分类列表")
	@RequestMapping("/productClassifyPageList/4.6.0")
	public BaseResponse productClassifyPageList460(AppRequestHead appRequestHead,ProductPageList4Request productPageList4Request){
		PaginatorResponse<ProductPageList4Response> rlt = cimProductService.queryProductPageList460(appRequestHead,productPageList4Request);
		return AppResponseUtil.getSuccessResponse(rlt);
	}
     ...
     */
	@ResponseBody
	@RequestLogging("产品可用加佣券")
	@RequestMapping("/productAddfeeTicket")
	public BaseResponse productAddfeeTicket(AppRequestHead head,ProductAddfeeTicketRequest productAddfeeTicketRequest){
		LOGGER.info("查询产品可使用的加佣券, productAddfeeTicketRequest={}", JSONObject.toJSONString(productAddfeeTicketRequest));
		//可使用的红包数量
		String userId = JsonWebTokenHepler.getUserIdByToken(head.getToken());//查询userId
		if(StringUtils.isNotBlank(userId) && !"undefined".equals(userId)){
			productAddfeeTicketRequest.setUserId(userId);
			return AppResponseUtil.getSuccessResponse(actPersonAddfeeTicketService.productPersonAddfeeTicket(productAddfeeTicketRequest));
		} else {
			return AppResponseUtil.getBaseResponse("100002", "token不能为空");
		}
	}	
