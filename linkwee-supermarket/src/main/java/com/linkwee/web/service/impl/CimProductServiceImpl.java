...
//			}
	...
	public List<ProductPageListResponse> queryProductClassifyPageListTop(AppRequestHead appRequestHead) {
		...
		productPageListResponses = cimProductMapper.queryHotRecommendPageListTop(productPageListRequest);
		
		//如果查询的产品不足10个  补全产品列表
		Integer limitSize = 10 - productPageListResponses.size();
		if(limitSize > 0){
			ProductPageListLimitRequest productPageListLimitRequest = new ProductPageListLimitRequest();
			BeanUtils.copyProperties(productPageListRequest, productPageListLimitRequest);
			productPageListLimitRequest.setLimitSize(limitSize);
			List<ProductPageListResponse>  addProductPageListResponseList = cimProductMapper.queryAddProductPageList(productPageListLimitRequest);
			//查询短期产品,高收益产品,中长期产品 主推平台
			List<String> recommendationPlatformList = cimProductCateService.selectRecommendationPlatform("3,4,5");
			Set<String> recommendationPlatformSet = new HashSet<String>();
			for (String recommendationPlatform : recommendationPlatformList) {
				recommendationPlatformSet.add(recommendationPlatform);
			}
			
			//按照主推平台的顺序排序
			Set<String>  productSet = new HashSet<String>();
			List<ProductPageListResponse> productPageListResponsesNew = new ArrayList<ProductPageListResponse>();
			for (ProductPageListResponse productPageListResponse1 : addProductPageListResponseList) {
				if(recommendationPlatformSet.contains(productPageListResponse1.getOrgNumber())){//主推平台放在最前面
					productPageListResponsesNew.add(productPageListResponse1);
					productSet.add(productPageListResponse1.getProductId());
				}
			}
			for (ProductPageListResponse productPageListResponse2 : addProductPageListResponseList) {
				if(!recommendationPlatformSet.contains(productPageListResponse2.getOrgNumber())){//非主推平台放在后面
					productPageListResponsesNew.add(productPageListResponse2);
					productSet.add(productPageListResponse2.getProductId());
				}
			}
			for (ProductPageListResponse productPageListResponse3 : productPageListResponses) { //最后补全最初查询的产品
				if(!productSet.contains(productPageListResponse3.getProductId())){
					productPageListResponsesNew.add(productPageListResponse3);
					productSet.add(productPageListResponse3.getProductId());
				}
			}		
			//添加产品标记
			addProductTags(productPageListResponsesNew,appType,userId);
			return productPageListResponsesNew;
		} else {
			//添加产品标记
			addProductTags(productPageListResponses,appType,userId);
	...
	@Override
	public PaginatorResponse<ProductPageList4Response> queryProductPageList4(AppRequestHead appRequestHead,ProductPageList4Request productPageList4Request) {
		String userId = JsonWebTokenHepler.getUserIdByToken(appRequestHead.getToken());
		productPageList4Request.setAppKind(AppUtils.getAppType(appRequestHead.getOrgNumber()).getValue());//设置app类型
		productPageList4Request.setIfHaveGray(sysGrayReleaseService.ifHaveGrayPermission(userId, "0,1"));//设置灰度权限
		
		PaginatorResponse<ProductPageList4Response> paginatorResponse = new PaginatorResponse<ProductPageList4Response>();
		Page<ProductPageList4Response> page = new Page<ProductPageList4Response>(productPageList4Request.getPageIndex(), productPageList4Request.getPageSize());
		List<ProductPageList4Response> productPageListResponses = cimProductMapper.queryProductPageList4(productPageList4Request,page);
		if(StringUtils.isBlank(userId) || "undefined".equals(userId)){
			for(ProductPageList4Response productPageList4Response:productPageListResponses){
				productPageList4Response.setHasRedPacket(false);
			}		
		}else {
			redPacketService.productRedPacketTag(productPageListResponses, userId);
		}
		paginatorResponse.setDatas(productPageListResponses);
		paginatorResponse.setValuesByPage(page);
		//针对玖富轻舟产品对于4.5.4版本以下用户都不显示
		String ver = appRequestHead.getAppVersion();
		if("android".equals(appRequestHead.getAppClient())||"ios".equals(appRequestHead.getAppClient())){
			if((ver!=null&&ver.length()>=5&&Integer.parseInt(ver.substring(0,1))<=4
					&&Integer.parseInt(ver.substring(2,3))<=5
					&&Integer.parseInt(ver.substring(4,5))<=4)||appRequestHead.getAppKind().indexOf("investor")!=-1){
				List<ProductPageList4Response> list = new ArrayList<ProductPageList4Response>();
				for (ProductPageList4Response pro : productPageListResponses) {
	...
	@Override
	public Integer productPageListStatistics4(AppRequestHead appRequestHead,ProductPageList4Request productPageList4Request) {
		productPageList4Request.setIfHaveGray(sysGrayReleaseService.ifHaveGrayPermission(JsonWebTokenHepler.getUserIdByToken(appRequestHead.getToken()), "0,1"));//设置灰度权限
		return cimProductMapper.productPageListStatistics4(productPageList4Request);
	}
	 ...
	 */
	@Override
	public List<ProductPageList4Response> querySelectedProducts(AppRequestHead head,ProductPageListRequest productPageListRequest) {
		List<ProductPageList4Response> productPageListResponses = cimProductMapper.querySelectedProducts(productPageListRequest);
		String userId = JsonWebTokenHepler.getUserIdByToken(head.getToken());
		if(StringUtils.isBlank(userId) || "undefined".equals(userId)){
			for(ProductPageList4Response productPageList4Response:productPageListResponses){
				productPageList4Response.setHasRedPacket(false);
	...
	@Override
	public PaginatorResponse<ProductPageList4Response> querySelectedProductsList(AppRequestHead head,SelectedProductsListRequest selectedProductsListRequest) {
		PaginatorResponse<ProductPageList4Response> paginatorResponse = new PaginatorResponse<ProductPageList4Response>();
		Page<ProductPageList4Response> page = new Page<ProductPageList4Response>(selectedProductsListRequest.getPageIndex(), selectedProductsListRequest.getPageSize());
		List<ProductPageList4Response> productPageListResponses = cimProductMapper.querySelectedProductsList(selectedProductsListRequest,page);
		String userId = JsonWebTokenHepler.getUserIdByToken(head.getToken());
		if(StringUtils.isBlank(userId) || "undefined".equals(userId)){
			for(ProductPageList4Response productPageList4Response:productPageListResponses){
				productPageList4Response.setHasRedPacket(false);
	...
	@Override
	public PaginatorResponse<OmsProductResponse> queryOmsProductList(OmsProductRequest omsProductRequest) {
		// TODO Auto-generated method stub
		List<OmsProductResponse> omsProductResponseList = new ArrayList<OmsProductResponse>();
		PaginatorResponse<OmsProductResponse> paginatorResponse = new PaginatorResponse<OmsProductResponse>();
		Page<OmsProductResponse> page = new Page<OmsProductResponse>(omsProductRequest.getPageIndex(), omsProductRequest.getPageSize());
		
		//查询机构产品sql获取产品信息
		CimOrgInfoToOur cimOrgInfoToOur = cimOrgInfoToOurService.selectOneByOrgNumber(omsProductRequest.getOrgNumber());
		if(cimOrgInfoToOur != null  && StringUtils.isNotBlank(cimOrgInfoToOur.getProductSql())){		
			omsProductResponseList = cimProductMapper.queryOmsProductList(cimOrgInfoToOur.getProductSql(),page);
		}
		
		paginatorResponse.setDatas(omsProductResponseList);
		paginatorResponse.setValuesByPage(page);
		return paginatorResponse;
	}
	
	@Override
	public List<ProductStatistics460Response> productClassifyStatistics460(AppRequestHead appRequestHead,ProductStatisticsRequest productStatisticsRequest) {
		List<ProductStatistics460Response> productStatisticsResponselist =  new ArrayList<ProductStatistics460Response>();
		String userId = JsonWebTokenHepler.getUserIdByToken(appRequestHead.getToken());//当前用户
		productStatisticsRequest.setIfHaveGray(sysGrayReleaseService.ifHaveGrayPermission(userId, "0,1"));
		productStatisticsResponselist = cimProductMapper.productClassifyStatistics460(productStatisticsRequest);
		return productStatisticsResponselist;
	}

	@Override
	public PaginatorResponse<ProductPageList4Response> queryProductPageList460(AppRequestHead appRequestHead,ProductPageList4Request productPageList4Request) {
		String userId = JsonWebTokenHepler.getUserIdByToken(appRequestHead.getToken());
		productPageList4Request.setAppKind(AppUtils.getAppType(appRequestHead.getOrgNumber()).getValue());//设置app类型
		productPageList4Request.setIfHaveGray(sysGrayReleaseService.ifHaveGrayPermission(userId, "0,1"));//设置灰度权限
		...
		PaginatorResponse<ProductPageList4Response> paginatorResponse = new PaginatorResponse<ProductPageList4Response>();
		Page<ProductPageList4Response> page = new Page<ProductPageList4Response>(productPageList4Request.getPageIndex(), productPageList4Request.getPageSize());
		List<ProductPageList4Response> productPageListResponses = cimProductMapper.queryProductCatePageList460(productPageList4Request,page);
		paginatorResponse.setDatas(productPageListResponses);
		paginatorResponse.setValuesByPage(page);
		//针对玖富轻舟产品对于4.5.4版本以下用户都不显示
		String ver = appRequestHead.getAppVersion();
