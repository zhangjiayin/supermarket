...
public class AcBankCardInfoServiceImpl extends GenericServiceImpl<AcBankCardInfo, Long> implements AcBankCardInfoService{
	...
	public void insertBankCard(AddBankCardRequest req) {
		...
		//设置默认头像
		int sex = this.getSexByIdcard(req.getIdCard());
		...
		if(cfp != null && cfp.getHeadImage() == null){
			...
			cfpUpdate.setUserId(req.getUserId());
...
//			conditionJson.put("mobile", req.getMobile());
			...
			String reslult = HttpRequestClient.invokeHttps(verifyConfig.getHttpUrl(), parameters, "UTF-8");
...
//			returnStr.put("code","请输入正确银行卡号");
			returnStr.put("code","00");//暂时先通过，只校验信用卡
        	return returnStr;
		}
