	...
	public BaseResponse pushProduct(ProductPushRequest productPushRequest,OpenRequestHead openRequestHead) {
		...
		try {
			...
			if(null == cimProduct){//插入产品信息
				...
				//20180803 小赢理财等额本息产品的基础佣金设为0.2%
				if("OPEN_XIAOYINGLICAI_WEB".equals(openRequestHead.getOrgNumber()) && productPushRequest.getRepaymentWay() == 2){
					cimProductNew.setFeeRatio(new BigDecimal(0.2));
				} else {			
    	...
    	try {
    		...
    		if(null != cimProduct){
    			...
    			if(cimProduct.getIsCollect() == 1 && productUpdatRequest.getStatus() == 3){
					...
					//只有推送过来的产品总额度不为空,则更新当前产品总额度
					if(productUpdatRequest.getBuyTotalMoney() != null){
					...
					//部分第三方平台会更新浮动利率
					if(productUpdatRequest.getFlowMinRate() != null){
						cimProductNew.setFlowMinRate(productUpdatRequest.getFlowMinRate());
					}
					if(productUpdatRequest.getFlowMaxRate() != null){
						cimProductNew.setFlowMaxRate(productUpdatRequest.getFlowMaxRate());
					}
