...
public class JsonWebTokenHepler {
	...
	public String creatToken(String appKind,String userId){
		return creatToken(appKind,userId,TimeSetConstants.TOKEN_VALID_DATE);
	}
	 ...
	 * @return
	 */
	public String creatToken(String appKind,String userId,long millisecond){
		String saveKey = "";
		String token = "";
		try {
			...
			long tokenValidityTime = System.currentTimeMillis()+millisecond;//有效期
			...
			if(jedisCluster != null){//若redis存在
				jedisCluster.setex(saveKey, (int)millisecond/1000,token);
			} else {
				//将token保存到tokenMap
	...
	public boolean resetTokenValidDate(String appKind,String token){
		...
		if(jedisCluster != null){//若redis存在
			jedisCluster.setex(saveKey, (int)TimeSetConstants.TOKEN_VALID_DATE_RESET/1000, token);
			return true;
		} else {
			if(tokenMap.containsKey(saveKey)){
				long tokenValidityTime = System.currentTimeMillis()+TimeSetConstants.TOKEN_VALID_DATE_RESET;//有效期
				tokenMap.put(saveKey, tokenValidityTime);
				return true;
			} else {
				return false;
