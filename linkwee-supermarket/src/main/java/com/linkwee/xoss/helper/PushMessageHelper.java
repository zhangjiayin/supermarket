...
public class PushMessageHelper {
	...
	public CommonTCSResult pushMessage(AppTypeEnum appType,
		...
		try {
			...
			SysPushParameter pushParam = new SysPushParameter();
			List<SysPushParameter> sysPushParameterList = sysPushParameterService.selectListByCondition(contition);
			if(CollectionUtils.isEmpty(sysPushParameterList)){
				throw new Exception("系统配置错误，未配置推送APP信息");
			} else {
				pushParam = sysPushParameterList.get(0);
			}
