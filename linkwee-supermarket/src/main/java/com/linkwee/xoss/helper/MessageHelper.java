...
public class MessageHelper {
	 ...
	 */
	public boolean sendVerifyCode(String mobile,AppTypeEnum appType,MsgModuleEnum msgModuleEnum,String verifyCode){
		BaseResponse baseResponse = sendVerifyCodeBaseResponse(mobile,appType,msgModuleEnum,verifyCode);
		if("0".equals(baseResponse.getCode())){
			return true;
		} else {
			return false;
	...
	public BaseResponse sendVerifyCodeBaseResponse(String mobile,AppTypeEnum appType,MsgModuleEnum msgModuleEnum){
		...
		return sendVerifyCodeBaseResponse(mobile,appType,msgModuleEnum,verifyCode);//发送验证码成功
	}
	 ...
	 * @param verifyCode	短信验证码
	 */
	public BaseResponse sendVerifyCodeBaseResponse(String mobile,AppTypeEnum appType,MsgModuleEnum msgModuleEnum,String verifyCode){
		...
		//发送短信
		CommonTCSResult retCode = messageQueueService.sendSingleMessage(mobile, appType, msgModuleEnum,verifyCode);
		if(retCode.getCode() != 0){		
			LOGGER.warn("发送短信异常,mobile={},appType={},msgModuleEnum={}",new Object[]{mobile,JSON.toJSONString(appType),JSON.toJSONString(msgModuleEnum)});
			boolean isNeedSendMsg = messageQueueService.isPreReturnMsg();
			if(isNeedSendMsg){
				return AppResponseUtil.getErrorBusi(String.valueOf(retCode.getCode()), retCode.getMessage());//发送验证码失败
			}
		}
