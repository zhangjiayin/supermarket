...
public class OpenHttpUtils {
	...
	public static String generateSignUrl(SysThirdkeyConfig sysThirdkeyConfig,String reqURL,Object... objectParams){
		...
		List<NameValuePair> formparams = changeRequest2NameValuePairForSign(sysThirdkeyConfig,objectParams);
		return getGetRequestUrl(reqURL,formparams);
	}
	...
	/**
	 * 根据请求url和请求参数拼接GET 请求URL
	 * @param reqURL
	 * @param nameValuePairList
	 * @return
	 */
	public static String getGetRequestUrl(String reqURL,List<NameValuePair> nameValuePairList){
		//生成完整的请求URL
		...
		try {
	        ...
	        requestURL.append(EntityUtils.toString(new UrlEncodedFormEntity(nameValuePairList)));
		} catch (Exception e) {
			LOGGER.info("根据请求URL和请求参数生成签名后的URL异常:{}",e);
		}
		
		String requestURLLast = requestURL.toString();
		LOGGER.info("根据请求url和请求参数拼接GET 请求最终  requestURLLast={}",requestURLLast);
		return requestURLLast;
	}
}
