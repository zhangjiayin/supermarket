...
/*	
     ...
     * @return
     */
	/*public WechatResp getAccessToken(String appid,String appsecret,int appType){
		WechatResp resp = new WechatResp();
		String accessToken;
		WxAccessToken wxAccessToken = wxAccessTokenService.selectNewAccessToken(appType);
    	if(wxAccessToken != null &&  (System.currentTimeMillis() - wxAccessToken.getCreateTime())/1000 < wxAccessToken.getExpiresIn()){
    		accessToken = wxAccessToken.getAccessToken();
    		resp.putData("token", accessToken);
    	} else {    			
            ...
            if (json.get("errcode") == null) {
    			...
    			if(StringUtils.isNotEmpty(accessToken)){				
    				...
    				wxAccessTokenN.setCreateTime(System.currentTimeMillis());
    				wxAccessTokenN.setExpiresIn((long) json.getIntValue("expires_in"));
    				wxAccessTokenN.setAppType(appType);
    				wxAccessTokenService.insert(wxAccessTokenN);
    			}
            } else {
 ...
 /** 
  ...
  /** 
     ...
     * @throws Exception
     */
    public WechatResp getWeChatShareResponse(CommonHelper commonHelper,String url,String appid,String appsecret,AppTypeEnum appTypeEnum) throws Exception {
        String timestamp = (System.currentTimeMillis() / 1000)+"";
        String nonceStr = UUID.randomUUID().toString();     
        //String accessToken = "";       
		//WechatResp tokenResp = getAccessToken(appid,appsecret,appTypeEnum.getKey());
		String accessToken = sysConfigService.getValuesByKey((appTypeEnum.getKey()==2?SysConfigConstant.TOOBEI_ACCTOKEN:SysConfigConstant.LIECAI_ACCTOKEN).trim());
        String signature = "";     
        WechatResp ticketResp = getJsTicket(commonHelper,accessToken,appTypeEnum);
	   	if(ticketResp.isSuccess()){
	   		...
	   		String swicth = sysConfigService.getValuesByKey(appTypeEnum.getKey()==2?SysConfigConstant.TOOBEI_WEIXIN_UPDATE_ACCTOKEN_SWICTH:SysConfigConstant.LIECAI_WEIXIN_UPDATE_ACCTOKEN_SWICTH);
			if(swicth!=null&&"ON".equals(swicth)){
		   		accessToken = weiXinMsgService.updateWeiXinAccToken(appTypeEnum.getKey());      	
	    		ticketResp = getJsTicket(commonHelper,accessToken,appTypeEnum);
	    		if(ticketResp.isSuccess()){
	       	   		 signature = getSignature(ticketResp.getString("ticket"), nonceStr, timestamp,url);
	       	   		 WechatResp resp = new WechatResp();
	       	   		 resp.putData("appid", appid);
    			...
    			logger.info("微信分享更新acctoken开关关闭，appType={}",appTypeEnum.getKey());
    			return ticketResp;
       	   	}		
	   	}
    }
