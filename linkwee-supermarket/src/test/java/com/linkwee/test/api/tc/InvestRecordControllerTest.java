...
public class InvestRecordControllerTest extends BaseTest{
	...
	@Test
	public void customerInvestTradeMsgTest() throws Exception{
		CustomerTradeMsgRequest customerTradeMsgRequest = new CustomerTradeMsgRequest();
		customerTradeMsgRequest.setType(1);
	//	customerTradeMsgRequest.setCustomerId(customerId);
		BaseResponse baseResponse = TestHelper.remote(AppEnum.CHANNEL_ANDROID,this.getUrl(),"/api/investRecord/cfplanner/customerTradeMsg",this.getToken(),BaseResponse.class);
		LOGGER.debug(baseResponse.toString());
	}	
	...
	@Test
	public void customerInvestProfit() throws Exception{
		BaseResponse baseResponse = TestHelper.remote(AppEnum.INVESTOR_ANDROID,this.getUrl(PathEnum.LOCALHOST),"/api/investRecord/customer/investProfit",this.getToken(),BaseResponse.class);
		LOGGER.debug(baseResponse.toString());
	}
