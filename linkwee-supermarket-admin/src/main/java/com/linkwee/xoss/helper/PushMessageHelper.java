...
public class PushMessageHelper {
	...
	public CommonTCSResult pushMessage(AppTypeEnum appType,
		...
		try {
			...
			SysPushParameter pushParam = sysPushParameterService.selectListByCondition(contition).get(0);
			if (pushParam == null) {
	...
	public CommonTCSResult pushMessageAsyn(AppTypeEnum appType,
		...
		try{
			...
			if (customerDevice == null) {
				...
				return new CommonTCSResult(-1, "无设备信息");
			}
			...
			//if(item.getStartType().intValue() == 0|| (item.getStartTime() !=null && dateResult != null && dateResult != -1)){//即时 或设置定时到指定时间
				...
				if(item.getSendType()!=null && item.getSendType().intValue() == 0){ //公告推送
					...
					if(ObjectUtils.equals(item.getSendObjectType(), 0)){ //全站推送
						...
						if(StringUtils.isNotBlank(item.getUserIds())&&item.getPushType()!=2){
					    ...
					    //个人中心消息
						if(item.getUserIds()!=null&&item.getPushType()!=1&&item.getSendObjectType()==1){
							List<SysMsg> msgs = Lists.newArrayList();
							String[] userIds  = item.getUserIds().replace("[", "").replace("]", "").replace("\"", "").split(",");
							for(String userId :userIds){
								SysMsg sysmsg = new SysMsg();
								sysmsg.setContent(item.getContent());
								sysmsg.setStatus(0);// 发布
								sysmsg.setUserNumber(userId);
	...
	public void pushSignRemind() {
		Map<String, Object> urlparam = Maps.newHashMap();
		urlparam.put("customUrlKey", SmsTypeEnum.SIGN_REMIND.getMsg());
		String content = configHelper.getValue(1,"sign_remind_message");
		List<String> userIdList = smCustomerDeviceService.hasNotSignedAndSignRemindOpenToday();
		if(userIdList != null && userIdList.size() > 0){
			BatchSinglePush(AppTypeEnum.CHANNEL, SmsTypeEnum.SIGN_REMIND,userIdList, "签到提醒", content, urlparam, false, null);
		}
	}
}
