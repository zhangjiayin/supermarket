...
public class CimProductInvestRecordServiceImpl extends GenericServiceImpl<CimProductInvestRecord, Long> implements CimProductInvestRecordService{
	...
	public List<InvestRecordWrapper> getInvestRecordByProducts(List<CimProduct> products){
		...
		for (CimProductInvestRecord investRecord : investRecords) {
			...
			//缓存key = 产品编号与日期
			String lockTimeKey = org.apache.commons.lang.StringUtils.join(new Object[]{"lockTime_",investRecord.getProductId(),format.format(investSuccessTime)});
			String maxTimeKey = org.apache.commons.lang.StringUtils.join(new Object[]{"maxTime_",investRecord.getProductId(),format.format(investSuccessTime)});
			//根据 key 获取  该笔投资的 的 解除日期 没有则计算 
			String lockTime = timeMaps.get(lockTimeKey);
			String maxTime = timeMaps.get(maxTimeKey);
			if(StringUtils.isBlank(lockTime)){
				...
				timeMaps.put(lockTimeKey, lockTime);	
			}
			if(StringUtils.isBlank(maxTime)){
				
				//起息日  = 投资成功时间或者募集成功时间 + 1 
				maxTime =format.format(org.apache.commons.lang.time.DateUtils.addDays(investSuccessTime, p.getDeadLineMaxValue()+1));
				timeMaps.put(maxTimeKey, maxTime);	
			}
			
			//当前日期大于解锁日期 创建投资记录包装类
			if(curTime.compareTo(lockTime)>0 && curTime.compareTo(maxTime)<0 ){
	...
	private InvestRecordWrapper createInvestRecordWrapper(CimProduct product,CimProductInvestRecord investRecord) {
		...
		investRecordWrapper.setStockInvestAmt(investRecord.getStockInvestAmt());
