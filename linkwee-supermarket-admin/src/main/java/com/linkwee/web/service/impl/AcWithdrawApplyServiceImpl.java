			    	...
			    	if("111".equals(pay.getStatus())&&"0".equals(with.getIsThaw())){
			    		...
			    		if(with != null && with.getUserType()!=null && messageTmp != null ){
			    			...
			    			if(with.getUserType().intValue() == 1){//理财师
			    				...
			    				SmMessageQueue sms = smMessageQueueService.fillSmMessageQueue(1,with.getMobile(),content,MsgModuleEnum.FINISHWITHDRAWAL);
			    				sms.setWithDrawTime(DateUtils.format(with.getCreatedDate(),DateUtils.FORMAT_LONG));
			    				sms.setMoney(NumberUtils.getFormat(with.getAmount(), "0.00"));
			    				lcsMessage.add(sms);
			    			}else{
			    				content = String.format(messageTmp.getContent(),with.getBisTime() == null ? "" : DateUtils.format(with.getBisTime(), DateUtils.FORMAT_SHORT_CN_MM),with.getAmount() == null ? "" : with.getAmount().setScale(2,BigDecimal.ROUND_DOWN),"猎财大师");
			    				invSysMsg.add(sysMsgService.fillSysMsg(2,PersonalMsgTypeEnum.WITHDRAWALS_INV,with.getUserId(),content));
			    				SmMessageQueue sms = smMessageQueueService.fillSmMessageQueue(2,with.getMobile(),content,MsgModuleEnum.FINISHWITHDRAWAL);
			    				sms.setWithDrawTime(DateUtils.format(with.getCreatedDate(),DateUtils.FORMAT_LONG));
			    				sms.setMoney(NumberUtils.getFormat(with.getAmount(), "0.00"));
			    				invMessage.add(sms);
			...
			public void run() {
				...
				if(lcsSysMsg.size() > 0){
					if("0".equals(channel)){
						...
						smMessageQueueService.batchSendDiffContentMessageAndPmsgJuHe(lcsMessage, true,lcsSysMsg);
					}
				}
				...
				if(invSysMsg.size() > 0){
					if("0".equals(channel)){
						...
						smMessageQueueService.batchSendDiffContentMessageAndPmsgJuHe(invMessage, true,invSysMsg);
					}
