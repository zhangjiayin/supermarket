		...
		if(swicth!=null&&"ON".equals(swicth)){
			...
			if(req!=null&&req.getUseId()!=null&&req.getTemkey()!=null&&rt!=null){
				...
				if("1".equals(req.getUseType())){
					...
					userName = lcs!=null?lcs.getUserName():null;
				}else{
					...
					userName = inv!=null?inv.getUserName():null;
				}
...
//			        	keyword4.setValue(req.getAmount());//回款额
			        	...
			        	keyword1.setValue(userName+"("+RandomTextCreator.encrypTion(mobile)+")");//用户姓名
			        	keyword2.setValue(req.getChangeType());//变更类型
			        	remark.setValue(req.getRemark());
			        ...
			        }else if(SysConfigConstant.CUSTOMER_BIND_SUCCESS.equals(req.getTemkey())
			        		||SysConfigConstant.SUPERIOR_BIND_SUCCESS.equals(req.getTemkey())
			        		||SysConfigConstant.SUBORDINATE_BIND_SUCCESS.equals(req.getTemkey())
			        		||SysConfigConstant.SUPERIOR_RELEASE_RELATIONSHIP.equals(req.getTemkey())
			        		||SysConfigConstant.SUBORDINATE_RELEASE_RELATIONSHIP.equals(req.getTemkey())
			        		||SysConfigConstant.CUSTOMER_RELEASE_RELATIONSHIP.equals(req.getTemkey())
			        		||SysConfigConstant.BIND_SUCCESS.equals(req.getTemkey())
			        		||SysConfigConstant.RELEASE_RELATIONSHIP.equals(req.getTemkey())){
			        	keyword1.setValue(req.getUserName());//用户名
			        	keyword2.setValue(req.getMobile());//手机号
