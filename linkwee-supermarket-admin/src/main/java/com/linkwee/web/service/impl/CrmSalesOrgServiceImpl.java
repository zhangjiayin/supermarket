...
import com.linkwee.web.request.PartnerRequest;
...
public class CrmSalesOrgServiceImpl extends GenericServiceImpl<CrmSalesOrg, Long> implements CrmSalesOrgService{
	...
	@Override
	public void salesOrgDepthJob() {
		CrmSalesOrg salesOrgRequest = new CrmSalesOrg();
		salesOrgRequest.setCooperationStatus(1);
		List<CrmSalesOrg> salesOrgList = crmSalesOrgMapper.selectByCondition(salesOrgRequest);
		for(CrmSalesOrg salesOrg : salesOrgList){
			String teamSql = generateTeamSql(salesOrg);
			PartnerRequest request = new PartnerRequest();
			request.setSalesOrgId(salesOrg.getSalesOrgId());
			request.setTeamSql(teamSql);
			int changeCount = crmSalesOrgMapper.partnerHasChanged(request);
			if(changeCount > 0){
				//先清空原有关系
				crmSalesOrgMapper.clearOldTeam(salesOrg.getSalesOrgId());
				//更新确认合伙人的团队及层级
				crmSalesOrgMapper.updatePartnerTeam(request);
			}
		}
	}

	@Override
	public String generateTeamSql(CrmSalesOrg salesOrg) {
		//合伙人团队层级
        ...
        sql.append(", case ");
        for( int i =1;i<n; i++){
        	sql.append(" when p"+i+".parent_id ='"+ salesOrg.getSalesOrgId()+"' THEN "+i);
        }
        ...
        sql.append(" where '"  + salesOrg.getSalesOrgId() +"' in (p1.parent_id ");
        for( int i =1+1;i<n; i++){
