...
/*	@Override
	...
	@Override
	public int stickNum(String appType) {		
		int stickNum = newsDao.stickNum(appType);
		return stickNum;
	}

	@Override
	public ReturnCode stickUpdate(String id, String isStick) {
		try {
			newsDao.stickUpdate(id,isStick);
			return new SuccessCode();
		} catch (Exception e) {
			logger.error("newsDao.stickUpdate invoke error:"+e.getMessage());
			e.printStackTrace();
			return Error.DB_ERROR;
		}
	}
