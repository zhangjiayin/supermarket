...
import com.linkwee.web.model.crm.InvestorManagerDetailRespNew;
...
public class InvestorManagerServiceImpl implements InvestorManagerService {
	...
	@Override
	public DataTableReturn queryInvestorListNew(DataTable dataTable, LcsListRequest request) {
		DataTableReturn tableReturn = new DataTableReturn();
		tableReturn.setDraw(dataTable.getDraw()+1);
		Page<InvestorManagerDetailRespNew> page = new Page<InvestorManagerDetailRespNew>(dataTable.getStart()/dataTable.getLength()+1,dataTable.getLength());
		List<InvestorManagerDetailRespNew> list = investorManagerDao.queryInvestorListNew(request, page);
		for(InvestorManagerDetailRespNew investorManagerDetailResp : list){
			investorManagerDetailResp.setCfplannerHeadImage(systemConfigService.getImageUrl(investorManagerDetailResp.getCfplannerHeadImage()));
		}
		tableReturn.setData(list);
		tableReturn.setRecordsFiltered(page.getTotalCount());
