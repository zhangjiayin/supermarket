    		...
    		LOGGER.debug("机构动态信息更新请求参数 CimOrginfoRequest = {}",JSON.toJSONString(request));
       		...
       		try {
       			...
       			if(jedisCluster != null){//若redis存在
       				String redisOrgKey = orginfo.getOrgNumber() + "_platform_detail_org_dynamic";
   	...
   	public Object deleteDynamic(Long id){
    	LOGGER.debug("删除机构动态 id = {}",JSON.toJSONString(id));
   		...
   		try {
   			//删除机构收费模式区间记录
   			if(null != id && id > 0){
   				CimOrgDynamic cimOrgDynamic = cimOrgDynamicService.selectById(id);
   				int deleteRows = cimOrgDynamicService.delete(id);
   				if(deleteRows > 0){
   					if(jedisCluster != null){//若redis存在
   	       				String redisOrgKey = cimOrgDynamic.getOrgNumber() + "_platform_detail_org_dynamic";
   	       				if(jedisCluster.exists(redisOrgKey)){
   	       					jedisCluster.del(redisOrgKey);//移除key
   	       				}
   	       			}
