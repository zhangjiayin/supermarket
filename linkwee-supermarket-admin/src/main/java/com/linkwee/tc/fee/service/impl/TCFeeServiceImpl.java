...
public class TCFeeServiceImpl extends GenericServiceImpl<TCFee, Long> implements TCFeeService{
	...
	public ResponseResult feeBalanceProcess() throws Exception{
		...
		boolean isSummary = feeBalanceService.isSummary(monthTrim, 1);
		if(isSummary)return new ResponseResult(false, "佣金已计算,不需要重复计算",feeBalanceService.getSummary(monthTrim));
		...
		if(feeSummaryTotalCount==0)return new ResponseResult(false, "佣金已计算,不需要重复计算",feeBalanceService.getSummary(monthTrim));
		...
		feeBalanceService.insertFeeSummary(monthTrim, date);
		return  result.isEmpty() ? new ResponseResult(true, "佣金计算成功",feeBalanceService.getSummary(monthTrim)) :new ResponseResult(false, "佣金计算异常,请联系开发处理");
	}
