...
public class TCFeeDetailServiceImpl extends GenericServiceImpl<TCFeedetail, Long> implements TCFeeDetailService{
	...
	public void insertFeedetail(FeedetailWrapper... feedetailWrappers) {
		try{
			...
			for (FeedetailWrapper feedetailWrapper : feedetailWrappers) {
				...
				feedetail.setProfitCfplannerId(feedetailWrapper.getProfitCfplannerId());
				feedetail.setOriginCfplannerId(feedetailWrapper.getOriginCfplannerId());
				...
				feedetail.setFeeRate(new BigDecimal(feedetailWrapper.getRatio()));
				feedetail.setFeeAmount(feedetailWrapper.getFeeamount());
				feedetail.setFeeType(feedetailWrapper.getFeetype());
				feedetail.setRemark(feedetailWrapper.getRemark());
	...
	@Transactional(propagation=Propagation.REQUIRED)
	@Override
	public int batchUpdateBalanceStatusBycfplannerIdAndFeeType(List<String> cfplannerIds, String feeType,List<Map<String, String>> balanceMaps, int balanceStatus,	String begintime, String endTime) {
		return feedetailMapper.batchUpdateBalanceStatusBycfplannerIdAndFeeType(cfplannerIds, feeType, balanceMaps, balanceStatus, begintime, endTime);
	}
