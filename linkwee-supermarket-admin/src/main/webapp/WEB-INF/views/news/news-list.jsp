...
<div id="main-news" class="container-fluid">
        ...
        <table id="J-newslist" class="table table-bordered" data-xtoolbars="#template-search"  data-url="rest/news/list_ajax" data-order="false" data-paging="true" data-size="10">
            <thead>
                <tr>
                    ...
                    <th data-name="isStick" data-format="isStick:stick_format">是否置顶</th>
    ...
    <script type="text/javascript">
	    ...
	    var stick_format = function (data,type,full,meta) {
	    	console.log(data);
	        if(data == 0){
	        	return '否';
	        }else{
        ...
        var news_opts  = function (data,type,full,meta) {
        	if(full.isStick == 0){
        		return '<a class="btn btn-sm btn-default btn-icon ui-redirect" href="javascript:;" data-title="资讯编辑" data-url="/rest/news/tosave?id='+data+'" ><i class="fa fa-edit"></i>编辑</a> &nbsp;&nbsp;<a class="btn btn-sm btn-danger btn-icon J_news_delete" data-id="'+data+'" href="javascript:;"><i class="fa fa-trash-o"></i>删除</a>&nbsp;&nbsp;<a class="btn btn-sm btn-success btn-icon J_news_stick" data-id="'+data+'" href="javascript:;"><i class="fa fa-hand-pointer-o"></i>选择置顶</a>';              
        	}else{
        		return '<a class="btn btn-sm btn-default btn-icon ui-redirect" href="javascript:;" data-title="资讯编辑" data-url="/rest/news/tosave?id='+data+'" ><i class="fa fa-edit"></i>编辑</a> &nbsp;&nbsp;<a class="btn btn-sm btn-danger btn-icon J_news_delete" data-id="'+data+'" href="javascript:;"><i class="fa fa-trash-o"></i>删除</a>&nbsp;&nbsp;<a class="btn btn-sm btn-success btn-icon J_news_deletestick" data-id="'+data+'" href="javascript:;"><i class="fa fa-hand-rock-o"></i>取消置顶</a>';                
        	}
        }
       ...
       var $db= $("#J-newslist").linkweeTable();
        ...
        $("#main-news").delegate("a.J_news_stick","click",function () {
            var iid = $(this).attr("data-id");          
            var appType = $("#news_app_type").val();
            $.get("rest/news/stickNum",{appType:appType},function (result) {
                if(result == 3){
                	bootbox.alert("置顶的资讯数量超过3个，请取消置顶资讯后再来操作！");
                }else{
                	$.get("rest/news/stickUpdate",{id:iid,isStick:1},function (result) {
                        if(result.isFlag){
        ...
        $("#main-news").delegate("a.J_news_deletestick","click",function () {
            var iid = $(this).attr("data-id");          
            var appType = $("#news_app_type").val();
            $.get("rest/news/stickNum",{appType:appType},function (result) {
                if(result.stickNum == 1){
                	bootbox.alert("置顶的资讯数量少于1个，不能进行该操作！");
                }else{
                	$.get("rest/news/stickUpdate",{id:iid,isStick:0},function (result) {
                        if(result.isFlag){
                        	bootbox.alert("操作成功",function () {
                        		$db.draw();
    						});
