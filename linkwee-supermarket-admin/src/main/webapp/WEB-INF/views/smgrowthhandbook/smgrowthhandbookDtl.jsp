...
<div class="container-fluid">
	<div class="row">
		<div class="col-sm-10">
			...
			<form id="xhandbookForm" action="rest/sm/smgrowthhandbook/save" method="post">
				<c:if test="${!empty handbook}">
					<input type="hidden" name="id" value="${handbook.id}"/>
				</c:if>
				<div class="form-group">
					<label>标题</label>
					<input name="title" type="text" class="form-control" value="${handbook.title}" placeholder="请输入资讯标题"  />
				</div>
				<div class="form-group">
					<label>类别</label>
					<select  name="typeCode" class="form-control">
						<c:forEach var="item" items="${growthHandbookClassifyList}"  varStatus="dn">
							<option <c:if test="${(not empty requestScope.handbook and requestScope.handbook.typeCode eq item.id)  }"> selected="selected"</c:if> value="${item.id }">${item.name }</option>
						</c:forEach>
					</select>
				</div>
				...
				<div class="form-group">
					...
					<select id="status" class="form-control" name="status">
						<option value="1" <c:if test="${handbook.status eq '1' }">selected</c:if> >有效</option>
						<option value="0" <c:if test="${handbook.status eq '0' }">selected</c:if> >无效</option>						
					</select>
				</div>
				...
				<div class="form-group">
					...
					<label><input name="cfpLevel" type="checkbox" 
						<c:forEach var="item" items="${cfpLevels}"  varStatus="dn">
							<c:if test="${('TA' eq item)  }"> checked="checked"</c:if> 
						</c:forEach>
					value="TA" />见习 </label> 
					<label><input name="cfpLevel" type="checkbox"
						<c:forEach var="item" items="${cfpLevels}"  varStatus="dn">
							<c:if test="${('SM1' eq item)  }"> checked="checked"</c:if> 
						</c:forEach>
					 value="SM1" />顾问 </label> 
					<label><input name="cfpLevel" type="checkbox"
						<c:forEach var="item" items="${cfpLevels}"  varStatus="dn">
							<c:if test="${('SM2' eq item)  }"> checked="checked"</c:if> 
						</c:forEach>
					 value="SM2" />经理 </label> 
					<label><input name="cfpLevel" type="checkbox" 
						<c:forEach var="item" items="${cfpLevels}"  varStatus="dn">
							<c:if test="${('SM3' eq item)  }"> checked="checked"</c:if> 
						</c:forEach>
					value="SM3" />总监</label> 
				</div>
				...
				<div class="form-group">
					<label>总阅读量</label>
					<input name="readingAmount" type="text" class="form-control" value="${handbook.readingAmount}" placeholder="请输入阅读量"  />
				</div>
					
				<div class="form-group">
					<label>摘要</label>
					<textarea id="summary" name="summary" class="form-control">${handbook.summary}</textarea>
				</div>	
				...
				<div class="form-group">
					<input name="img" type="hidden" value="${handbook.img}"/>
					<div class="row" id="images-list">
						<c:if test="${ !empty handbook.img}">
							<img src="${img_server}${handbook.img}" id="img-thumbnail" class="image thumbnail" onerror="javascript:$(this).remove();" style="width: 300px;" />
						</c:if>
					</div>
				</div>
				...
				<div class="form-group">
					<input name="shareIcon" type="hidden" value="${handbook.shareIcon}"/>
					<div class="row" id="icons-list">
						<c:if test="${ !empty handbook.shareIcon}">
							<img src="${img_server}${handbook.shareIcon}"  id="icon-thumbnail" style="width: 300px;" />
						</c:if>
					</div>
				</div>
				...
				<div class="form-group">
					<label>来源</label>
					<input name="source" type="text" class="form-control" value="${handbook.source}"  placeholder="请输入发布来源" />
				</div>
				...
				<div class="form-group" id="contentId">
					<label>正文</label>
					<textarea id="content" name="content">${handbook.content }</textarea>
				</div>
				<div class="form-group" style="display: none;" id="linkUrlId">
					<label>链接</label>
					<input name="linkUrl" class="form-control"  value="${handbook.linkUrl }" />
				</div>
	...
	jQuery.validator.addMethod("checkPicSize", function(value,element) {
		...
		var ue = UE.getEditor('content',{serverUrl:"rest/sm/smgrowthhandbook/ueditor_config",customDomain:true,
			zimg:zimgUrl,
			zIndex:9996,
						...
						success:function (result) {
							if(result.isFlag){
								bootbox.alert(result.msg,function () {
									$.switchPage("成长手册列表","rest/sm/smgrowthhandbook/list");
								});
							}
							else{
