...
<div class="container-fluid">
	<div class="row">
		<div class="col-sm-10">
			...
			<form id="xforgeInvestAmountForm" action="rest/act/actforgeinvestamount/save" method="post">
				<c:if test="${!empty actForgeInvestAmount}">
					<input type="hidden" name="id" value="${actForgeInvestAmount.id}"/>
				</c:if>
				<div class="form-group">
					<label>姓名</label>
					<input name="userName" type="text" class="form-control" value="${actForgeInvestAmount.userName }"  placeholder="请输入姓名" />
				</div>
				<div class="form-group">
					<label>电话</label>
					<input name="mobile" type="text" class="form-control" value="${actForgeInvestAmount.mobile}" placeholder="请输入电话"  />
				</div>
				<div class="form-group">
					<label>总投资额</label>
					<input name="investAmt" type="text" class="form-control" value="${actForgeInvestAmount.investAmt}" placeholder="请输入总投资额"  />
				</div>
				<div class="form-group">
					<label>图像</label>
				...
				<div class="form-group">
					...
					<div class="row" id="images-list">
						<c:if test="${ !empty actForgeInvestAmount.headImage}">
							<img src="${actForgeInvestAmount.headImage}" id="img-thumbnail" class="image thumbnail" onerror="javascript:$(this).remove();" style="width: 300px;" />
						</c:if>
					</div>
				</div>
						...
						success:function (result) {
							if(result.isFlag){
								bootbox.alert(result.msg,function () {
									$.switchPage("数据列表","rest/act/actforgeinvestamount/list");
								});
							}
							else{
