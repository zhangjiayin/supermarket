...
<form id="addfeeTicketFrom">
	...
	<div class="row">
		...
		<div class="col-md-4">
			<input type="text" class="form-control" name="name" value="${addfeeTicket.name}" autocomplete="off" placeholder="加拥券名称" />
		</div>
	</div>
	<br>
	<div class="row">
		...
		<div class="col-md-4">
			<input type="text" class="form-control" name="rate" value="${addfeeTicket.rate}" autocomplete="off" placeholder="加拥比率" />
		</div>
	</div>
	<br>
	<div class="row">
		...
		<div class="col-md-4">
			<input type="text" class="form-control" name="remark" value="${addfeeTicket.remark}" autocomplete="off" placeholder="加拥券描述" />
		</div>
	</div>
	<br>
	<div class="row">
	...
	<div class="row">
		...
		<div class="col-md-2">
			<select name="platformLimit" id="platformLimit" onchange ="changePlatformLimit()" class="form-control">
				<option value="0" <c:if test="${addfeeTicket.isPlatformLimit() == false}">selected="selected"</c:if> >不限</option>
				<option value="1" <c:if test="${addfeeTicket.isPlatformLimit() == true}">selected="selected"</c:if> >限制</option>
			</select>
		</div>
		<div class="col-md-2">
			<select class="form-control" id="platformLimitOrgNumber" name="platformLimitOrgNumber">
				...
				<c:forEach var="platform" items="${platformList}" >
					<option value="${platform.orgNumber}" <c:if test="${addfeeTicket.platformLimitOrgNumber==platform.orgNumber }">selected="selected"</c:if> >${platform.name}</option>
				</c:forEach>
			</select>
		</div>
	</div>
	...
	<div class="row">
		...
		<div class="col-md-2">
			<select name="investLimit" class="form-control">
				<option value="0" <c:if test="${addfeeTicket.investLimit==0}">selected="selected"</c:if> >不限</option>
				<option value="1" <c:if test="${addfeeTicket.investLimit==1}">selected="selected"</c:if> >用户首投</option>
				<option value="2" <c:if test="${addfeeTicket.investLimit==2}">selected="selected"</c:if> >平台首投</option>
			</select>
		</div>
	</div>
	<br>
	<div class="row">
		...
		<div class="col-md-2">
			<select name="amountLimit" id="amountLimit" onchange ="changeAmountLimit()" class="form-control">
				<option value="0" <c:if test="${addfeeTicket.amountLimit==0}">selected="selected"</c:if> >不限</option>
				<option value="1" <c:if test="${addfeeTicket.amountLimit==1}">selected="selected"</c:if> >大于</option>
				<option value="2" <c:if test="${addfeeTicket.amountLimit==2}">selected="selected"</c:if> >大于等于</option>
			</select>
		</div>
		<div class="col-md-2">
			<input type="text" class="form-control" name="amount" id="amount" value="${addfeeTicket.amount}"  autocomplete="off" placeholder="投资金额" />
		</div>
	...
	<div class="row">
		...
		<div class="col-md-2">
			<select name="productLimit" id="productLimit" onchange ="changeProductLimit()" class="form-control">
				<option value="1000" <c:if test="${addfeeTicket.productLimit==1000}">selected="selected"</c:if> >不限</option>
				<option value="1002" <c:if test="${addfeeTicket.productLimit==1002}">selected="selected"</c:if> >等于</option>
				<option value="1003" <c:if test="${addfeeTicket.productLimit==1003}">selected="selected"</c:if> >大于等于</option>
			</select>
		</div>
		<div class="col-md-2">
			<input type="text" class="form-control" name="productLimitDeadline" id="productLimitDeadline" value="${addfeeTicket.productLimitDeadline}" autocomplete="off" placeholder="投资期限  (单位 ：天)" />
		</div>
	</div>
	<br>
	<div class="row">
		...
		<div class="col-md-2">
			<select name="addFeeLimit" id="addFeeLimit" onchange ="changeAddFeeLimit()" class="form-control">
				<option value="0" <c:if test="${addfeeTicket.addFeeLimit==0}">selected="selected"</c:if> >不限</option>
				<option value="1" <c:if test="${addfeeTicket.addFeeLimit==1}">selected="selected"</c:if> >限制</option>
			</select>
		</div>
		<div class="col-md-2">
			<input type="text" id="addfeeDays" class="form-control" name="addFeeLimitDay"  value="${addfeeTicket.addFeeLimitDay}" autocomplete="off" placeholder="加拥天数" />
		</div>
	</div>
	<br>
	<div class="row">
		<div class="col-md-2 col-md-offset-4">
			<a class="btn btn-danger active J_updateAddFeeTicket"  role="button"  >保存</a>
			&nbsp;&nbsp;&nbsp;&nbsp;
...
$(document).ready(function() {
    ...
    $(".J_updateAddFeeTicket").on("click", function(event) {
    	...
    	$.post('rest/cim/actpersonaddfeeticket/edit', data, function(data, textStatus, xhr) {
            layer.msg(data.msg,{time: 1000,icon: data.isFlag?1:0});
            if( data.isFlag)$.switchPage("个人加拥券管理","rest/cim/actpersonaddfeeticket/initPage"); //跳到个人加拥券列表页面
        });
        return false;
    });

    $(".J_goback").on("click", function(event) {
        $.switchPage("个人加拥券管理","rest/cim/actpersonaddfeeticket/initPage"); //跳到个人加拥券列表页面
        return false;
    });
