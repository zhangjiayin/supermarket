...
<div class="container-fluid">
	<div class="row">
		<div class="col-sm-10">
			...
			<form id="editSmbrandpromotionForm" action="rest/acc/smbrandpromotion/save" method="post">
				<c:if test="${!empty smbrandpromotion}">
					<input name="id" type="hidden" value="${smbrandpromotion.id}"/>
				</c:if>
				...
				<div class="form-group">
					...
					<div class="row" id="images-list">
						<c:if test="${ !empty smbrandpromotion.image}">
							<img src="${smbrandpromotion.image}" id="img-thumbnail" class="image thumbnail" onerror="javascript:$(this).remove();" style="width: 300px;" />
						</c:if>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="row" id="images-showIcon">
						<c:if test="${ !empty smbrandpromotion.smallImage}">
							<img src="${smbrandpromotion.smallImage}" id="img-thumbnail" class="image thumbnail" onerror="javascript:$(this).remove();" style="width: 300px;" />
						</c:if>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="row">
						<div class="col-sm-6">
							<input id="validBegin" name="validBegin" class="Wdate" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'#F{$dp.$D(\'validEnd\')}'})"  style="width:150px" value="${smbrandpromotion.validBegin }" />
						</div>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="row">
						<div class="col-sm-6">
							<input id="validEnd" name="validEnd" class="Wdate" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'#F{$dp.$D(\'validBegin\')}'})"  style="width:150px" value="${smbrandpromotion.validEnd }" />
						</div>
					</div>
				</div>
				...
				<div class="form-group">
					<label>显示顺序</label>
					<input id="showInx" name="showInx" class="form-control"  value="${smbrandpromotion.showInx }" />
				</div>
						...
						success:function (result) {
							...
							if(result.isFlag){
								bootbox.alert(result.msg,function () {
									$.switchPage("个人推广海报 ","rest/acc/smbrandpromotion/list");
								});
							}
							else{
