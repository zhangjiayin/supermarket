...
<input type="hidden" id="isCpaAndCps" value="${isCpaAndCps}" />
	...
	<form id="orgFeeForm" action="/rest/cim/cimorginfo/updateOrgFee" method="post">
		...
		<div class="row">
				<div class="page-header">
					<span  style="color: blue;font-size: 15px;">该机构旗下的所有产品都按以下方式对机构进行收费、对理财师进行结佣。<br />CPA+CPS未选中且CPA、CPS同时选中时，用户首次投资时，销售费用使用CPA收费模式进行计算；<br />CPA+CPS选中且CPA、CPS同时选中时，用户首次投资时，销售费用使用CPA+CPS收费模式进行计算；<br />用户第二次投资及以后的投资，使用CPS收费模式进行计算。</span>
				</div>
				
				<div class="col-md-9 page-header checkbox">
                		<label>
                    		<input type="checkbox" id="cpacpscbo" name="cpaAndCpsType" value="cpaAndCps" /><strong>CPA+CPS&nbsp;(首笔投资同时按CPA+CPS计算销售费用)</strong>
                    	</label>
                </div>
												  		...
												  		<c:if test="${feeRecord.feeAttr eq 'year_propertion'}">
												  			<tr id="feeRecords-100${idx.count}">
															    ...
															    <td class="col-md-8">
															      ...
															      <td>
															      		<div class="form-group" style="margin-top: 10px;">
																	      <input type="text" class="form-control text-center" min="0" number="true" required="required" name="orgFeeRecords_feeRecords100${idx.count}.moneyRatio" value="<fmt:formatNumber value="${feeRecord.feeRatio}" pattern="0.00" minFractionDigits="0" />" autocomplete="off"  />
																	    </div>
												  	...
												  	<c:forEach var="feeRecord" items="${orgFeeList}" varStatus="idx">
												  		<c:if test="${feeRecord.feeAttr eq 'month_amount_propertion'}">
													  		<tr id="feeRecords-200${idx.count}">
															      ...
															      <td>
															      		<div class="form-group" style="margin-top: 10px;">
																	      <input type="text" class="form-control text-center" min="0" number="true" required="required" name="orgFeeRecords_feeRecords200${idx.count}.moneyRatio" value="<fmt:formatNumber value="${feeRecord.feeRatio}" pattern="0.00" minFractionDigits="0" />" autocomplete="off"  />
																	    </div>
															      </td>
															      <td>
																	    <div style="margin-top: 10px;">
