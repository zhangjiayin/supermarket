...
<div id="main-adv" class="container-fluid">
    ...
    <script type="text/javascript">
        ...
        //未回款投资总额
		function showNoReturnAmount(){
    		 var startDate =$('#startDate').val();
             var endDate =$('#endDate').val();
             var orgNumber =$('#orgNumber').val();
            
    		$.ajax({
    				url :"rest/orgsalefee/noreturn_amount",
    				type : 'post',
    				data:{'startDate':startDate,'endDate':endDate,'orgNumber':orgNumber},					
    				dataType : "json",
    				success : function(data) {
    					$("#noReturnAmount").html("<font color='red'>浮动期未回款总额："+data+"</font>");
    				},
    ...
    <script type="text/linkwee-template" id="template-search">
   	...
   	日期<input id="startDate" name="startDate" class="form-control Wdate"
        ...
        <div id = "noReturnAmount" style="width: 350px;display: inline-block;text-align: center;"></div>
		<shiro:hasPermission name="<%=PermissionSign.ORG_SALE_STATISTIC_EXPORT%>">
			<div id = "exportBtnDiv" style="width: 300px;display: inline-block;float: right;">
				<button class="btn btn-default btn-sm J_export" role="button"  style="float: right;">导出数据</button>
			</div>
		</shiro:hasPermission>
	</form>
    </script>
