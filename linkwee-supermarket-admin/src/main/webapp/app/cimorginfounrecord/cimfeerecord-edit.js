$(function(){
	    ...
	    submitHandler: function (form) {
	    	$.ajax({
	            ...
	            success: function (result) {
	            	if(result.isFlag){
						...
						layer.alert(result.msg,{icon: 6,shift: 4,title:'提示'},function(index){
							layer.close(index);
							$.switchPage("A专区平台","rest/cim/cimorginfoa/list"); //跳到机构列表页面
						});
					}else{
						//新增失败
	            ...
	            error:function(XmlHttpRequest,textStatus, errorThrown) {
	  			  	...
	  			  	showError(XmlHttpRequest.responseText+"更新机构返现策略失败！");
	  		  	}
	          });
	    }
	});
	...
	function addFirstInvestRow(addBtn){
		...
		var firstInvest_index = parseInt($(addBtn).attr("data-index")); //指定首投期限区间行下标 默认：NaN
	...
	function appendFirstInvestHtml(index) {
		...
		content.push('<input type="text" class="form-control text-center" name="orgFeeRecords_feeRecords' + index + '.feeRatio" autocomplete="off" min="0" number="true" required="required" />');
		content.push('</div></td>');
	...
	function deleteFirstInvestRow(delBtn){
		...
		layer.confirm('确定删除此行？', {btn: ['确定','取消'],title:'提示',shift: 6,icon: 3}, function(){
			...
			var rowid = $(delBtn).attr("data-feerowid");
		  	...
		  	if(!$(delBtn).attr("data-feeid")){
				...
				$("#"+rowid).remove(); //删除此行
		  		...
		  		$.ajax({
		            ...
		            dataType:'json',
		            type:'post',
		            url : '/rest/cim/cimorginfoa/deleteFeeStrategy',
		            success: function (result) {
		            	if(result.isFlag){
		            		layer.msg('删除此条记录成功！', {icon: 1,time:500});
							$("#"+rowid).remove(); //删除此行
