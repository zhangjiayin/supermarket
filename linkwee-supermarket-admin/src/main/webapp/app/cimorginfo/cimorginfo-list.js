   ...
   var table = $('#dtable').DataTable({
        ...
        "columns": [
            ...
            { "data": "recommend" ,"render": function ( data, type, row ) {
             ...
             { "data": "listRecommend" ,"render": function ( data, type, row ) {
           	   if(data=="1")
           		   return "推荐";
           	   else if(data=="0")
           		   return "不推荐";
...
/*            { "data": "top" },*/
            { "data": "status" ,"render": function ( data, type, row ) {
              ...
              { "data": "id" ,"orderable": false,"render": function ( data, type, row ) {
            	    ...
            	    var b = '<button class="btn btn-default btn-sm J_recommendInfo" data-toggle="modal" data-target="#recommendModal" data-url="/rest/cim/cimorginfo/toRecommend?id='+row.id+'">排名/推荐</button>&nbsp';
            	    var c = '<a href="javacript:void(0);" class="btn btn-default btn-sm J_feeRatio" data-orgNumber="'+row.orgNumber+'">佣金</a>&nbsp';
	  	            ...
	  	            var f = '<a href="javacript:void(0);" class="btn btn-sm btn-default btn-icon ui-redirect" data-title="机构风控" data-url="/rest/cim/cimorginfo/toRiskHandle?orgNumber='+row.orgNumber+'"><i class="fa fa-hand-o-right"></i>风控</a>';
	  	            var result = '';
	  	            if(row.feeModelEditPermission == '1'){
	  	            	result += a;
	  	            }
	  	            result += b;
	  	            if(row.feeEditPermission == '1'){
	  	            	result += c;
	  	            }
	  	            result += d;
	...
	$("#dtable").off('click','.J_recommendInfo').on('click','.J_recommendInfo',function(event){
		...
		$.ajax({
			...
			success : function(result) {
				...
				if(recommend == 1){
					$("#homePageRecommend").attr("checked","checked");
				}else if(recommend == 0){
					$("#homePagenoRecommend").attr("checked","checked");
				}
				var listRecommend = result.data.listRecommend;
				if(listRecommend == 1){
					$("#listRecommend").attr("checked","checked");
				}else if(listRecommend == 0){
					$("#listnoRecommend").attr("checked","checked");
				}
