    ...
    var $db = $("#J_teamList_list").DataTable(
            {
                ...
                columns:[
                    {data:"cfpName"},
                    {data:"cfpMobile",render: function ( data, type, row ) {
                    	return data.replace(/^(\d{3})\d{4}(\d+)/,"$1****$2");
                	}},
                    {data:"city"},
                    {data:"customerName"},
                    {data:"customerMobile",render: function ( data, type, row ) {
                    	return data.replace(/^(\d{3})\d{4}(\d+)/,"$1****$2");
                	}},
                    {data:"investAmt"},
                    {data:"platfromName"},
 		 ...
 		 <div class="col-md-9">
	      	<select name="platfrom" id="platfrom" class="form-control">
				...
				<c:forEach items="${platfroms}"  var="platfrom" >
					<option value="${platfrom.org_number }">${platfrom.org_name }</option>
				</c:forEach>
			</select>
	    </div>
	</div>
