...
public class AuthController {
    ...
    @Resource
    private CimOrginfoService userService;
    ...
    public String login(@Validated CimOrginfo user, BindingResult result, Model model, HttpServletRequest request) {
        ...
        // 验证成功在Session中保存用户信息
        final CimOrginfo authUserInfo = userService.selectOne(user);
    ...
    public Result resetPwd(@RequestParam("password")String password,@RequestParam("new_password")String new_password) {
    	...
    	CimOrginfo u = (CimOrginfo)session.getAttribute("userInfo");
    	if(!u.getOrgPassword().equals(password)){
    		return new Result(false,401,"原密码不匹配！");
    	}
    	u.setOrgPassword(new_password);
