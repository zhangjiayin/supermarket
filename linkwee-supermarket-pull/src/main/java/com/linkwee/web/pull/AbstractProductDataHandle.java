				...
				if(productDataPullReturn.getSysThirdkeyConfigPull() != null){
					...
					for(CimProductAddPull cimProductAddPullNewf:cimProductAddPullListNew){
						...
						if(!isExist){
							...
							//如果售罄  查询售罄产品的详情
							CimProductAddPull cimProductAddPull = pullProductById(orgNumber,cimProductAddPullNewf.getThirdProductId());
							
							
							//没有异常 并且有销售结束时间
							if(cimProductAddPull != null && StringUtils.isNotBlank(cimProductAddPull.getSaleEndTime())){
								
								cimProductAddPullNewf.setStatus(cimProductAddPull.getStatus());
								
								//避免第三方判断错误  只有状态是2-售罄或者3-募集失败时  才更新产品售罄时间
								if(StringUtils.isNotBlank(cimProductAddPull.getSaleEndTime())&& (cimProductAddPull.getStatus() == 2 || cimProductAddPull.getStatus() == 3)){
									cimProductAddPullNewf.setSaleEndTime(cimProductAddPull.getSaleEndTime());	
								}
								cimProductAddPullNewf.setBuyedTotalMoney(cimProductAddPull.getBuyedTotalMoney());
								cimProductAddPullNewf.setBuyedTotalPeople(cimProductAddPull.getBuyedTotalPeople());
								...
								} else if(cimProductAddPull.getStatus() == 1){
									cimProductAddPullNewf.setRemark("拉取系统在售产品不在获取产品列表里面   单独查询id进行更新");
								} else {
									cimProductAddPullNewf.setRemark("单独查询id进行更新异常");
								}
...
//								//五星财富单独处理
//								if("OPEN_WUXINGCAIFU_WEB".equals(orgNumber)){								
//									cimProductAddPullNewf.setBuyedTotalMoney(cimProductAddPullNewf.getBuyTotalMoney());
//									cimProductAddPullNewf.setStatus(2);
//									cimProductAddPullNewf.setSaleEndTime(dateNow);
//									
//									//产品更新表
//									CimProductUpdatePull cimProductUpdatePullforInsertf = new CimProductUpdatePull();
//									BeanUtils.copyProperties(cimProductAddPullNewf, cimProductUpdatePullforInsertf);
...
//									cimProductAddPullNewf.setUpdater(orgNumber);
//									cimProductAddPullNewf.setUpdateTime(dateNow);
//									cimProductAddPullNewf.setRemark("产品被第三方删除  直接更新为售罄");
//									cimProductAddPullService.update(cimProductAddPullNewf);
//								}
							}
						}
